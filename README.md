...
- **Cash Flow & Reconciliation (營運收支中央看板)** — Monthly Product Sales / Other Income / Gross Revenue cards, an "Add Income" modal (category, date, amount, account, remarks, compressed voucher upload), a unified ledger of all revenue (Product Sale vs Other Income badges) with receipt thumbnails and Pending/Verified toggles, plus **Bank Statement CSV/Excel import** with auto-reconciliation (reference match → Bank Cleared, amount+date fuzzy match → accountant confirmation, unmatched → Unclaimed Deposits Pool)
