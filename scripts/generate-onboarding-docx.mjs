...
function buildChildren() {
  ...
  children.push(
    ...
    headerTable(
      ...
      [
        ...
        ['Orders & activity', 'orders, order_files, activity_logs (+ unused legacy order_activities)'],
        ['Quotations', 'quotations, quotation_items, quotation_files'],
        ['Cash flow', 'other_income'],
        ['Kitchen', 'kitchen_finished, kitchen_raw, kitchen_daily_orders, kitchen_batches, kitchen_gift_boxes, kitchen_movements, kitchen_order_fulfillments, kitchen_prep_orders, kitchen_settings'],
  ...
  children.push(
    ...
    h3('Kitchen (/kitchen) — inventory & fulfillment'),
    bullet('Two-tier inventory: kitchen_finished + kitchen_raw (available = total − allocated).'),
    bullet('Order routing: if finished stock ≥ qty → deduct + Ready to Ship; else 無現貨 backlog.'),
    bullet('Manual batches: allocate raw → complete → consume raw, add finished, FIFO backlog.'),
    bullet('Mutating APIs return full fresh state. Lib: kitchen.ts + kitchen-server.ts.'),
    ...
    bullet('Import from bird-nest order → calculate → print → complete (yield reporting) → activity on linked order + finished stock.'),
    ...
    bullet('API: /api/settings/integrations; QuickBooks connect/callback/status'),
    ...
    bullet('System events use author System (reminders, hub, etc.). Legacy order_activities is unused.'),
  ...
  children.push(
    ...
    numbered('Configure stores under Settings → Integrations (or env overlays).'),
    numbered('Manual: /hub UI or POST /api/hub/sync/woocommerce|quickbooks; date-range import under /api/hub/import/[platform].'),
    ...
    numbered('Create/edit rental unit (lease master: rent, due day, automation toggles).'),
    numbered('Monthly rental_records generated for billing periods; amount can be overridden.'),
    numbered('Send invoice email (POST …/records/[id]/invoice); mark paid with optional receipt (…/paid).'),
    numbered('Cron: /api/cron/rental-invoices → runRentalInvoiceDispatch().'),
    ...
    numbered('Import: POST /api/kitchen-prep/import (or cron kitchen-prep-import) from Nestiee/wedding order quantities.'),
