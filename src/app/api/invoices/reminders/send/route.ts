...
export async function POST(request: Request) {
  ...
  const inv = getInvoiceWithDetails(invoiceId, ownerId);
  ...
  if (type === 'due_soon') {
    ...
    db.prepare("UPDATE invoices SET last_reminder_at = datetime('now') WHERE id = ? AND user_id = ?").run(
      invoiceId,
      ownerId,
    );
  }
  ...
  logActivity('invoice', invoiceId, session.userId, 'activity', session.name, msg);
  if (inv.order_id) {
    logActivity('order', inv.order_id, session.userId, 'activity', session.name, msg);
  }
