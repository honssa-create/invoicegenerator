...
export async function entityBelongsToUser(type: EntityType, entityId: number | string, userId: number): Promise<boolean> {
  const table = OWNER_TABLE[type];
  if (!table) return false;
  const row = await db.prepare(`SELECT 1 FROM ${table} WHERE id = ? AND user_id = ?`).get(entityId, userId);
  return Boolean(row);
}
