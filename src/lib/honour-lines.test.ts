...
describe('appendHonourShippingLine', () => {
  const product = { style: '燙貼織嘜', quantity: '300', unit_price: '861' };
  ...
  it('appends Shipping when total > 0', () => {
    expect(appendHonourShippingLine([product], 35)).toEqual([
      product,
  ...
  it('replaces an existing Shipping row', () => {
    ...
    expect(appendHonourShippingLine(withOld, 20)).toEqual([
      product,
...
describe('extractHonourCpoOptions / applyHonourOptionsToFields', () => {
  ...
  it('maps Honour-specific craft labels to their related fields', () => {
    const fields: Record<string, unknown> = {};
    ...
    expect(String(fields.other_craft)).toContain('金屬電鍍色: 古銅色');
    expect(String(fields.other_craft)).toContain('背面配件: 蝴蝶扣');
    expect(String(fields.other_craft)).toContain('做法: 滴膠');
  });
