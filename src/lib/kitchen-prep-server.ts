...
/**
 * Upsert open 補充存貨 prep rows for finished-bottle shortfalls (one per capacity).
 * Matches linked_order_id IS NULL + order_type restock + same capacity + open status.
 */
export async function upsertRestockPrepsForShortfalls(
  ownerId: number,
  groups: { capacity: PrepCapacity; qtys: { osmanthus: number; red_date: number; rock_sugar: number } }[]
): Promise<RestockPrepRef[]> {
  const today = hkTodayIso();
  const out: RestockPrepRef[] = [];

  for (const g of groups) {
    const o = Math.max(0, Math.round(g.qtys.osmanthus));
    const r = Math.max(0, Math.round(g.qtys.red_date));
    const s = Math.max(0, Math.round(g.qtys.rock_sugar));
    if (o + r + s <= 0) continue;
    ...
    const existing = (await db
      .prepare(
        `SELECT * FROM kitchen_prep_orders
         WHERE user_id = ?
           AND order_type = 'restock'
           AND linked_order_id IS NULL
           AND capacity = ?
           AND status IN ('inactive', 'scheduled', 'in_prep')
         ORDER BY id ASC
         LIMIT 1`
      )
      .get(ownerId, g.capacity)) as PrepRow | undefined;

    let prep: PrepOrder | null;
    if (existing) {
      prep = await updatePrepOrder(existing.id, ownerId, {
        stewing_date: today,
        order_type: 'restock',
        capacity: g.capacity,
        status: 'in_prep',
        qty_osmanthus: Math.max(existing.qty_osmanthus, o),
        qty_red_date: Math.max(existing.qty_red_date, r),
        qty_rock_sugar: Math.max(existing.qty_rock_sugar, s),
        allowEmptyQtys: false,
      });
    } else {
      prep = await createPrepOrder(ownerId, {
        stewing_date: today,
        order_type: 'restock',
        capacity: g.capacity,
        qty_osmanthus: o,
        ...
        status: 'in_prep',
        notes: 'Auto: gift-box packaging finished shortfall',
        allowEmptyQtys: false,
      });
    }
